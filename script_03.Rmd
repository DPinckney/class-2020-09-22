...
# be thoughtful about what we are excluding
# conscious and specific about what we are focusing
# careful about assumptions we are making
# make assumptions that will allow future selves to manage the question and
# the data



After we have a model, Temperance suggests we should:
# be aware of the limitations of the data and its validity
# exercise self-control, discretion, and moderation
...
```{r}
sep_combined <- inner_join(sep_new, sep_old, by = "title") %>%
  rename(enrollment_new = enrollment.x, enrollment_old = enrollment.y) %>%
  mutate(enrollment_change = (enrollment_new - enrollment_old)) %>%
  arrange(enrollment_change)

five_decrease <- sep_combined %>%
  slice(1:5)

five_increase <-sep_combined %>%
  arrange(desc(enrollment_change)) %>%
  slice(1:5)

ten_extremes <- rbind(five_increase, five_decrease) %>%
  mutate(increase = if_else (enrollment_change > 0, "TRUE", "FALSE"))

ggplot(ten_extremes, mapping = aes(y= enrollment_change, x = title, fill = increase)) +
  geom_col() +
  labs(title = "Classes with September Enrollment Changes", x = "Class", y = "Change in Enrollment", subtitle = "Top 10 classes with most extreme enrollment changes") +
  theme(axis.text.x = element_text(size = 9, angle = 45)) +
  scale_fill_discrete(name = "Enrollment Change", labels = c("Decrease", "Increase"))
```
